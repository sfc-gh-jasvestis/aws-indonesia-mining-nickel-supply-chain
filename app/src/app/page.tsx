...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Shipments (MTD)" value={kpiVal('Shipments (MTD)', '84 vessels')} status="neutral" />
        <KPICard title="Port Queue" value={kpiVal('Port Queue', '7 days')} status="danger" />
        <KPICard title="Ore Stockpile" value={kpiVal('Ore Stockpile', '2.4M WMT')} status="neutral" />
        <KPICard title="Active Haulers" value={kpiVal('Active Haulers', '1,247')} status="neutral" />
      </div>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="lg:col-span-1">
          <GeoMap
            country="indonesia"
            labels={{ entity: 'Vessels', event: 'Production Logs', alert: 'Delays' }}
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'WMT (K)' }]}
          title="Ore Movement Volume (Weekly)"
        />
        <Chart
          ...
          title="Logistics Cost by Corridor"
        />
      </div>
        </div>
      </div>
      <DataTable
        columns={[
          ...
          { key: 'm1', header: 'Cargo (WMT)' },
          { key: 'm2', header: 'Hauling Cost Wmt' },
          { key: 'm3', header: 'Port Utilization' },
          { key: 'events', header: 'Production Logs' },
          { key: 'alerts', header: 'Delays' },
        ]}
        data={data?.entities || []}
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Hauling Cost/WMT" value={kpiVal('Hauling Cost/WMT', 'Rp 124K')} />
        <KPICard title="Port Utilization" value={kpiVal('Port Utilization', '87%')} />
        <KPICard title="Vessel Turnaround" value={kpiVal('Vessel Turnaround', '4.2 days')} />
      </div>
      <Chart
        data={data?.detail || [{ x: 'Mon', y: 24 }, { x: 'Tue', y: 28 }, { x: 'Wed', y: 22 }, { x: 'Thu', y: 31 }, { x: 'Fri', y: 26 }, { x: 'Sat', y: 19 }, { x: 'Sun', y: 23 }]}
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'Rp K/WMT' }]}
        title="Transport Cost Breakdown"
        height={400}
      />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          xKey="label"
          yKeys={[{ key: 'value', name: 'WMT (K)' }]}
          title="Buyer Demand Forecast"
        />
        <ActionMemo
          persona={{ name: 'Agus Prabowo', role: 'Supply Chain Director' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Resolve port congestion at Obi Island jetty', 'Secure additional barges for Halmahera route', 'Negotiate Q4 shipping rates with China buyers'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which ports have the longest vessel queue?',
          'Show ore stockpile levels vs shipping schedule',
          'What is the optimal inventory buffer at each port?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Logistics', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Demand Planning', icon: '⚡', content: domainTab2 },
